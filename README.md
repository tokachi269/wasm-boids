...
- **build / refit**  
  Morton オーダーで Boid を並べ替えた線形 BVH を構築し、毎フレームは葉ノードの AABB だけを更新する「refit」で済ませます。これにより大規模シーンでも再構築コストを抑えられます。

- **gatherNearest**  
  近傍探索は near-first の深さ優先探索を採用しています。ノード距離を使って枝刈りし、候補は固定長ヒープに蓄積 → 最後に距離昇順で返却します。葉内では距離²の途中計算で早期スキップするなど、算術量を減らす工夫を入れています。

- **SoA との直接連携**  
  `SoABuffers` の位置配列を直接参照する設計です。Boid の追加コピーを発生させないよう意図されたもので、README 本文で述べた群れロジックと密接に結び付いています。

...
void setGlobalSpeciesParamsFromJS(
...
void callInitBoids(const std::vector<SpeciesParams> &speciesParamsList,
                   float spatialScale = 1.0f, float posRange = 1.0f,
                   float velRange = 1.0f) {
...
SimulationTuningParams getSimulationTuningParams() { return gSimulationTuning; }
...
void setSimulationTuningParams(const SimulationTuningParams &params) {
  ...
  gSimulationTuning.maxEscapeWeight =
      std::clamp(gSimulationTuning.maxEscapeWeight, 0.0f, 1.0f);
  gSimulationTuning.separationMinFactor =
      std::clamp(gSimulationTuning.separationMinFactor, 0.0f, 1.0f);
  gSimulationTuning.antiMillGain =
      std::clamp(gSimulationTuning.antiMillGain, 0.0f, 1.0f);
}
...
EMSCRIPTEN_BINDINGS(my_module) {
  ...
  value_object<SimulationTuningParams>("SimulationTuningParams")
      ...
      .field("escapeStrengthPerThreat",
             &SimulationTuningParams::escapeStrengthPerThreat)
      .field("cohesionBoost", &SimulationTuningParams::cohesionBoost)
      .field("separationMinFactor",
             &SimulationTuningParams::separationMinFactor)
      .field("alignmentBoost", &SimulationTuningParams::alignmentBoost)
      .field("antiMillGain", &SimulationTuningParams::antiMillGain);
  ...
  class_<BoidTree>("BoidTree")
      ...
      .function("getOrientationsPtr", &BoidTree::getOrientationsPtr)
  ...
  function("callInitBoids", &callInitBoids);
}
